...
interface DispatchStore {
  ...
  canAssignTo: (personnelId: string) => boolean;
...
function isPersonAvailable(p: Personnel): boolean {
  if (p.status === 'offline' || p.status === 'away' || p.status === 'busy') return false;
  return true;
}
...
const DEFAULT_STATE = {
  ...
  lastAutoAssignLog: null as { ticketId: string; assignedTo: string; reason: string; time: number } | null,
...
export const useDispatchStore = create<DispatchStore>()(
  persist(
    (set, get) => ({
      ...
      canAssignTo: (personnelId) => {
        const p = get().getPersonnel(personnelId);
        if (!p) return false;
        return isPersonAvailable(p);
      },
      ...
      getCandidatesFor: (ticketId, topN = 5) => {
        ...
        if (!ticket) return [];
        const isCritical = ticket.level === 'P0' || ticket.level === 'P1';
        const result: CandidateScore[] = [];
        for (const p of get().personnel) {
          if (p.status === 'offline') continue;
          const available = isPersonAvailable(p);

          if (isCritical && !available) continue;
          ...
          const availFactor = p.status === 'away' ? -20 : p.status === 'busy' ? -10 : 0;
      ...
      autoDispatch: (ticketId) => {
        ...
        const available = cands.filter(c => c.available);
        if (available.length === 0) return { assigned: false, reason: '无可用维修人员' };
      ...
      assignTicket: (ticketId, assigneeId, byId, byName) => {
        ...
        if (!isPersonAvailable(person)) return false;
        const now = Date.now();
      ...
      acceptTicket: (ticketId, byId, byName) => {
        ...
        set(prev => {
          ...
          return {
            tickets: newTickets,
            personnel: syncPersonnelWithTickets(
              prev.personnel.map(p =>
