...
export const TopBar: React.FC = () => {
  ...
  return (
    ...
    `}>
      ...
      <div className="flex items-center gap-1.5 mr-3 rounded-lg border border-cyber-cyan/30 bg-cyber-card/50 p-0.5">
        <Link
          to="/"
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md transition ${loc.pathname === '/' ? 'bg-cyber-cyan/20 text-cyber-cyan shadow-neon-cyan' : 'text-slate-400 hover:text-cyber-cyan hover:bg-cyber-cyan/10'}`}
        >
          <Monitor size={12} />
          <span className="text-[10px] font-mono tracking-wider">监控台</span>
        </Link>
        <Link
          to="/dispatch"
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md transition ${loc.pathname === '/dispatch' ? 'bg-cyber-cyan/20 text-cyber-cyan shadow-neon-cyan' : 'text-slate-400 hover:text-cyber-cyan hover:bg-cyber-cyan/10'}`}
        >
          <ArrowRightLeft size={12} />
          <span className="text-[10px] font-mono tracking-wider">派工墙</span>
        </Link>
      </div>
