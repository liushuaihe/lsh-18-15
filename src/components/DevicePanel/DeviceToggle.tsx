...
import { Zap, AlertTriangle, Lock, Link, Power, Cpu } from 'lucide-react';
            ...
            {canShowChainButton && (
              <button
                ...
                className="flex items-center justify-center gap-1 w-12 h-7 rounded text-[10px] font-mono
                  ...
                  hover:bg-cyber-cyan/20 hover:shadow-neon-cyan transition-all"
                title="一键启动依赖链"
              >
                <Power size={11} />
                链启
...
function r_isError(msg: string): boolean {
  return msg.includes('失败') || msg.includes('未就绪') || msg.includes('阻断') || msg.includes('请先') || msg.includes('熔断') || msg.includes('故障');
}
